...
let addedLiblinkListener = false;
const setUpInstantLiblink = () => {
    if (addedLiblinkListener) return;
    addedLiblinkListener = true;
    document.addEventListener('click', event => {
        if (event.target.host !== 'liblink.pl') return;
        if (location.href === 'https://portal.librus.pl//vendor/widget-librus/index.html') {
        ...
        setTimeout(() => {
            const a1 = Array.from(document.querySelectorAll('.dialog'));
            a1[a1.length - 1].remove();
            const a2 = Array.from(document.querySelectorAll('.dialog-mask'));
            a2[a2.length - 1].remove();
            const a3 = Array.from(document.getElementsByTagName('ons-dialog'));
            a3[a3.length - 1].remove();
        }, 200);
        return false;
    });
};
